...
class data_generator():
    def __init__(self, img_path, save_path, pre_augment_mode = 0, augment_mode = 0, 
                    img_res = (128, 128), ifx2 = True, inter_method = 'Fourier', 
                    sliding_interval = 64, gen_mode='local'):
        """
        ...
        inter_method
            ...
            {default: 'Fourier'}
        gen_mode
            'local': local patch generation;
            'global': global patch generation;
            {default: 'local'}
    ...
    def interpft(self, x, ny, dim = 0):
        ...
        if dim >= 1: 
            # f interpolating along columns, dim = 1
    ...
    def savedata(self, image_stack, flage):
        """
        Adaptive SelfN2N tool: TO save data
        ...
        for s in range(slices):    
            ...
            # Normalization
            if self.gen_mode == 'local':
                img = img * 255  # Denormalize from 0-1 to 0-255
            elif self.gen_mode == 'global':
                pass
    ...
    def savedata4folder(self, threshold_mode, interval = 64, threshold = 15):
        """
    ...
    def savedata4folder_agument(self, flage = 1, interval = 64, threshold_mode = 2, threshold = 15, 
        ...
        """
        Adaptive SelfN2N tool: Generator with random interchange
        ------
        flage
            started file index
            {default: 1}
        ...
        # If pre_augment_mode is 0, no preprocessing is performed
        if self.pre_augment_mode == 0:
            times = 0
            roll = 0
        
        ## Read all files under the folder
        datapath_list = []  # Used to store file paths
        ...
        l = len(datapath_list)  # Number of files in the folder

        ## Process files one by one
        for ll in range(l):
            try:
                image_data = self.imread_stack(datapath_list[ll])
                print('Processing frame %d'%(ll + 1))

                # Process 3D images
                try:
                    [t, x, y] = image_data.shape  # Get the dimensions of the image
                    
                    # Iterate through each slice of each image
                    for taxial in range(t):
                        # Save original data
                        image_arr = self.slidingWindow(image_data[taxial,:,:],
                        ...
                        # Randomly interchange ROIs
                        for circlelarge in range(roll):
                            if times >= 1:
                                # Randomly select an image from another time point for interchange
                                ...
                                # Repeat the interchange for times-1 times
                                for circle in range(times - 1):
                                    image_data_pre = self.random_interchange(imga = image_data_pre,
                                        imgb = image_data[random.randint(0, t - 1),:,:], 
                                            size = size , mode = self.pre_augment_mode)
                                # Apply sliding window to the augmented image
                                image_arr = self.slidingWindow(image_data_pre, 
                                        threshold_mode = threshold_mode, threshold = threshold)
                                # Save augmented data
                                flage = self.savedata(image_arr, flage)
                # Otherwise process 2D images
                except ValueError:
                    # If pre_augment_mode is 3, randomly select another image for interchange
                    ...
                    for circlelarge in range(roll):
                        if times >= 1:
                            # Randomly select another image for interchange
